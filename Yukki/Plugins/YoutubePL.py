from Yukki.Utilities.yt_playlist import get_yt_playlist
import random
...
@app.on_callback_query(filters.regex("ytpl"))
async def play_youtube_playlistcb(_, CallbackQuery):
    try:
        global get_queue
            ...
            if for_p == 1:
                ...
                if await isPreviewUp(preview):
                    await CallbackQuery.message.reply_photo(
                        photo=preview,
                        caption=f"**This is Queued Youtube Playlist.**\n\nPlayed by :- {CallbackQuery.from_user.mention}",
                        quote=False,
                        reply_markup=InlineKeyboardMarkup(buttons),
                    )
                    await m.delete()
